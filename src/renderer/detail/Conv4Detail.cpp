...
bool Conv4Detail::initialize() {
    initializeHotspots();
    std::cout << "conv4热点交互初始化完成，热点数量: " << hotspots_.size() << std::endl;
     
     // 创建动画器
        animator = std::make_unique<Conv4Anim>();
        
        // 加载数据
        bool success = animator->load("/workspace/assets/model");
        
        std::cout << "conv4热点交互初始化完成" 
                  << (success ? " (包含动画)" : " (动画加载失败)") << std::endl;

    return true;
}

void Conv4Detail::initializeHotspots() {
...
std::string Conv4Detail::getButtonText(const std::string& hotspotName) const {
    // 根据热点名称返回对应的按钮文本
    if (hotspotName == "conv4_kernel") {
        return "卷积动画";
...
void Conv4Detail::drawHotspots(ImVec2 contentSize, ImVec2 imagePos) {
    /*
    // 绘制热点区域（调试用）
...
void Conv4Detail::drawHotspot(const Hotspot& hotspot, ImVec2 contentSize, ImVec2 imagePos) {

    std::cout << "Conv4Detail::drawHotspots called. Hotspot count: " << hotspots_.size() << std::endl;
...
void Conv4Detail::handleMouse(const sf::Vector2f& mousePos, ImVec2 contentSize, ImVec2 imagePos) {
    // 重置悬停状态
...
void Conv4Detail::drawButton(const Hotspot& hotspot, ImVec2 contentSize, ImVec2 imagePos) {
    // 计算热点区域的中心位置
...
void Conv4Detail::handleButtons() {
